module GPUIntersection6D
    ...
    function prepare_simplices_for_gpu(P::Matrix{Int}, S_indices::Vector{NTuple{7, Int}})
        num_simplices = length(S_indices)
        # SMatrix{7 (Ecken), 6 (Dimensionen), Int64, 42 (Elemente)}
        simplices_data = Vector{SMatrix{7, 6, Int64, 42}}(undef, num_simplices)
        for i in 1:num_simplices
            cpu_matrix_f64 = Int64.(P[collect(S_indices[i]), :])
            simplices_data[i] = SMatrix{7, 6, Int64, 42}(cpu_matrix_f64)
        end
        return CuArray(simplices_data)
    end

    # --- 6D-Vektoroperationen (Int64) ---
    ...
    @inline function normal_vector_6d(v1, v2, v3, v4, v5)
        ...
        return SVector{6, Int64}(c1, c2, c3, c4, c5, c6)
    end
    ...
    function get_intersecting_pairs_gpu_6d(P::Matrix{Int}, S_indices::Vector{NTuple{7, Int}})
        num_simplices = length(S_indices)
