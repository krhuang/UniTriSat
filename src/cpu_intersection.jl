# cpu_intersection.jl
#
# Public API is byte-for-byte compatible with the previous version:
#   * same export list (get_intersecting_pairs_cpu is exported-but-undefined, as before)
#   * Simplex{V, D} -- two type parameters, unchanged
#   * prepare_simplices_cpu(P::Matrix{Int}, S_indices::Vector, ::Val{D}) -> Vector{Simplex{D+1,D}}
#   * get_intersecting_pairs_cpu_generic(...)  -> Vector{Vector{Int}} of [-i, -j]
#   * check_intersections_range_cpu(...)       -> Vector{Vector{Int}} of [-i, -j]
#   * find_intersections_in_subset(...)        -> Vector{Vector{Int}} of [-i, -j]
#   * simplices_intersect_sat_cpu(s1, s2)      -> Bool
#   * compute_face_normal(face_verts, Val(D))
#
# Changes are all internal:
#
...
# ---------------------------------------------------------------------------
#
# NB: call sites pass the minor in row-major order while the generated parameter
# list is column-major, i.e. the callee sees the transpose.  det(A') = det(A),
# so this is harmless -- but it is load-bearing, do not "fix" it.
